...
const homeSpotlights = [
  { label: 'Fresh from the water', title: 'Today’s catch is in.', detail: 'Cleaned catfish from Ogbia farmers', action: 'deals', icon: '🐟' },
  { label: 'Just harvested', title: 'New greens for your table.', detail: 'Fresh ugu leaves from Yenagoa gardens', action: 'new-arrivals', icon: '🥬' },
  { label: 'Local pantry', title: 'Stock up and save.', detail: 'Better prices on Bayelsa staples', action: 'categories', icon: '🧺' },
]
...
function CategoriesPage({ categories, onBack, onSelectCategory }) {
  return (
    <main className="category-page">
      ...
      <nav className="nav category-page-nav"><button className="back-button" onClick={onBack}>← Back to home</button><a className="brand" href="#top" onClick={onBack}><span className="brand-mark">✳</span><span>AgriMarket <i>Bayelsa</i></span></a><div className="nav-actions"><button className="join-button" onClick={onBack}>Shop fresh produce <span>↗</span></button></div></nav>
      <section className="category-page-hero">
        <p className="eyebrow">Explore the market</p>
        <h1>Find something fresh<br /><em>for every table.</em></h1>
        <p>Browse produce, fish, pantry staples, and farm goods from trusted sellers across Bayelsa.</p>
      </section>
      <section className="category-page-content">
        <div className="section-heading"><div><p className="eyebrow">Shop by collection</p><h2>What are you looking <em>for?</em></h2></div><span className="category-count">{categories.length} collections</span></div>
        <div className="category-page-grid">{categories.map((category) => <button className={`category ${category.color}`} onClick={() => onSelectCategory(category.name)} key={category.name}><span>{category.icon}</span><strong>{category.name}</strong><small>Browse products →</small></button>)}</div>
      </section>
      <footer><a className="brand" href="#top" onClick={onBack}><span className="brand-mark">✳</span><span>AgriMarket <i>Bayelsa</i></span></a><p>Better food. Stronger communities.</p><span>© 2024 AgriMarket Bayelsa</span></footer>
    </main>
  )
}

function CheckoutPanel({ cart, total, onClose, onComplete }) {
  const [submitted, setSubmitted] = useState(false)
  return (
    <div className="overlay checkout-overlay" onClick={onClose}>
      <section className="checkout-panel" onClick={(event) => event.stopPropagation()}>
        ...
        {submitted ? <div className="checkout-success"><span>✓</span><p className="eyebrow">Order received</p><h2>Your basket is on its way.</h2><p>We’ll confirm your delivery details and send an update shortly.</p><button className="primary-button" onClick={onClose}>Back to shopping <span>↗</span></button></div> : <><p className="eyebrow">Secure checkout</p><h2>Complete your order.</h2><div className="checkout-summary"><strong>{cart.length} {cart.length === 1 ? 'item' : 'items'}</strong><span>₦{total.toLocaleString()}</span></div><form className="checkout-form" onSubmit={(event) => { event.preventDefault(); setSubmitted(true); onComplete() }}><label>Full name<input required placeholder="e.g. Amarachi Peter" /></label><label>Phone number<input required type="tel" placeholder="+234 800 000 0000" /></label><label>Delivery address<textarea required placeholder="House number, street, area, Yenagoa" /></label><div className="form-grid"><label>Delivery area<select defaultValue="Yenagoa"><option>Yenagoa</option><option>Ogbia</option><option>Sagbama</option><option>Nembe</option><option>Brass</option></select></label><label>Payment method<select defaultValue="card"><option value="card">Card payment</option><option value="transfer">Bank transfer</option><option value="cash">Pay on delivery</option></select></label></div><button className="primary-button full checkout-submit" type="submit">Place order · ₦{total.toLocaleString()} <span>↗</span></button></form></>}
      </section>
    </div>
  )
}

function ProductCollectionPage({ title, eyebrow, description, products, searchQuery, cart, total, favorites, onBack, onAddToCart, onRemoveFromCart, onToggleFavorite, onClearCart }) {
  const visibleCollectionProducts = products.filter((product) => !searchQuery || `${product.name} ${product.detail} ${product.farmer}`.toLowerCase().includes(searchQuery.toLowerCase()))
  const [showCart, setShowCart] = useState(false)
  const [showCheckout, setShowCheckout] = useState(false)
  return (
    <main className="category-page collection-page">
      <div className="topline"><span>📍 Yenagoa, Bayelsa</span><span>Free delivery on orders over ₦20,000 · Mon - Sun: 8:00 AM - 10:00 PM</span><span>☎ (123) 456-7890</span></div>
      <nav className="nav category-page-nav"><button className="back-button" onClick={onBack}>← Back to home</button><a className="brand" href="#top" onClick={onBack}><span className="brand-mark">✳</span><span>AgriMarket <i>Bayelsa</i></span></a><div className="nav-actions"><button className="collection-cart" onClick={() => setShowCart(true)} aria-label="Open cart">♧<sup>{cart.length}</sup></button><button className="join-button" onClick={onBack}>Shop home <span>↗</span></button></div></nav>
      <section className="category-page-hero collection-hero"><p className="eyebrow">{eyebrow}</p><h1>{title}</h1><p>{description}</p></section>
      <section className="category-page-content"><div className="section-heading"><div><p className="eyebrow">From verified local sellers</p><h2>{searchQuery ? `Results for “${searchQuery}”` : 'Good food, ready for your basket.'}</h2></div><span className="category-count">{visibleCollectionProducts.length} products</span></div><div className="product-grid collection-grid">{visibleCollectionProducts.length ? visibleCollectionProducts.map((product) => <article className="product-card" key={product.name}><div className="product-image" style={{ backgroundImage: `url(${product.image})` }}><span className="badge">{product.badge}</span><button className={`heart ${favorites.includes(product.name) ? 'is-favourite' : ''}`} onClick={() => onToggleFavorite(product.name)} aria-label={`${favorites.includes(product.name) ? 'Remove' : 'Add'} ${product.name} ${favorites.includes(product.name) ? 'from' : 'to'} favourites`}>{favorites.includes(product.name) ? '♥' : '♡'}</button></div><div className="product-info"><div className="product-title"><h3>{product.name}</h3><span>⋮</span></div><p>{product.detail}</p><div className="farmer"><span className="farmer-avatar">{product.farmer.charAt(0)}</span><span><strong>{product.farmer} <b>✓</b></strong><small>⌖ {product.location}</small></span></div><div className="price-row"><strong>{product.price}</strong>{product.old && <del>{product.old}</del>}<button onClick={() => onAddToCart(product)} aria-label={`Add ${product.name} to cart`}>+</button><button className="buy-button" onClick={() => { onAddToCart(product); setShowCart(true) }}>Buy now</button></div></div></article>) : <p className="empty-results">No products are available here yet.</p>}</div></section>
      <footer><a className="brand" href="#top" onClick={onBack}><span className="brand-mark">✳</span><span>AgriMarket <i>Bayelsa</i></span></a><p>Better food. Stronger communities.</p><span>© 2024 AgriMarket Bayelsa</span></footer>
      {showCart && <div className="overlay" onClick={() => setShowCart(false)}><aside className="drawer" onClick={(event) => event.stopPropagation()}><button className="close" onClick={() => setShowCart(false)}>×</button><p className="eyebrow">Your basket</p><h2>Fresh picks.</h2>{cart.length === 0 ? <div className="empty"><span>🧺</span><p>Your basket is waiting for something good.</p></div> : <>{cart.map((item, index) => <div className="cart-item" key={`${item.name}-${index}`}><img src={item.image} alt="" /><div><strong>{item.name}</strong><small>{item.price}</small></div><button className="cart-remove" onClick={() => onRemoveFromCart(index)} aria-label={`Remove ${item.name} from cart`}>×</button></div>)}<div className="cart-total"><span>Total</span><strong>₦{total.toLocaleString()}</strong></div><button className="primary-button full" onClick={() => { setShowCart(false); setShowCheckout(true) }}>Continue to checkout →</button></>}</aside></div>}
      {showCheckout && <CheckoutPanel cart={cart} total={total} onClose={() => setShowCheckout(false)} onComplete={onClearCart} />}
    </main>
  )
}

function LogisticsDashboard({ onBack }) {
  const [filter, setFilter] = useState('All')
  const [bookingOpen, setBookingOpen] = useState(false)
  const [booked, setBooked] = useState(false)
  const shipments = [
    { id: 'AM-2048', customer: 'Chinonso E.', route: 'Yenagoa → Amarata', status: 'In transit', driver: 'Success A.', eta: 'Today, 2:40 PM' },
    { id: 'AM-2051', customer: 'Bright Kitchen', route: 'Ogbia → Yenagoa', status: 'Ready', driver: 'Assign driver', eta: 'Today, 4:00 PM' },
    { id: 'AM-2041', customer: 'Daniel K.', route: 'Sagbama → Yenagoa', status: 'Delivered', driver: 'Timi B.', eta: '19 Jun, 11:18 AM' },
    { id: 'AM-2037', customer: 'Faith A.', route: 'Nembe → Yenagoa', status: 'Delayed', driver: 'Timi B.', eta: 'Tomorrow, 9:30 AM' },
  ]
  const visibleShipments = shipments.filter((shipment) => filter === 'All' || shipment.status === filter)

  return <main className="logistics-page"><aside className="logistics-sidebar"><a className="brand" href="#logistics"><span className="brand-mark">✳</span><span>AgriMarket <i>Logistics</i></span></a><div className="logistics-operator"><span className="profile-avatar">OP</span><div><strong>Operations desk</strong><small>Bayelsa region</small></div></div><nav className="logistics-nav"><a className="active" href="#logistics">▦ <span>Overview</span></a><a href="#shipments">⌁ <span>Shipments</span><b>12</b></a><a href="#drivers">♧ <span>Drivers</span></a><a href="#zones">⌖ <span>Delivery zones</span></a></nav><div className="logistics-health"><strong>Dispatch health</strong><span><i></i> All systems online</span><small>Last sync · 2 minutes ago</small></div><button className="logistics-back" onClick={onBack}>↩ Back to marketplace</button></aside><section className="logistics-content" id="logistics"><header className="logistics-header"><div><p className="eyebrow">Thursday, 20 June 2024 · Live operations</p><h1>Move every order <em>with care.</em></h1><p>Coordinate pickups, drivers, and doorstep deliveries across Bayelsa.</p></div><button className="logistics-primary" onClick={() => { setBooked(false); setBookingOpen(true) }}>+ Book a delivery</button></header><div className="logistics-metrics"><article><strong>12</strong><small>Active deliveries</small><b>↑ 3 today</b></article><article><strong>5</strong><small>Ready for pickup</small><b>Needs assignment</b></article><article><strong>8 / 10</strong><small>Drivers on route</small><b>80% capacity</b></article><article><strong>94.6%</strong><small>On-time rate</small><b>↑ 2.1% this week</b></article></div><section className="logistics-board" id="shipments"><div className="logistics-board-heading"><div><p className="eyebrow">Dispatch board</p><h2>Today’s shipments</h2></div><div className="shipment-filters">{['All', 'Ready', 'In transit', 'Delayed'].map((option) => <button className={filter === option ? 'selected' : ''} onClick={() => setFilter(option)} key={option}>{option}</button>)}</div></div><div className="shipment-head"><span>Shipment</span><span>Route</span><span>Driver</span><span>Status</span><span>ETA</span></div>{visibleShipments.map((shipment) => <article className="shipment-row" key={shipment.id}><div><strong>#{shipment.id}</strong><small>{shipment.customer}</small></div><span>{shipment.route}</span><span>{shipment.driver}</span><b className={`shipment-status ${shipment.status.toLowerCase().replace(' ', '-')}`}>{shipment.status}</b><span>{shipment.eta}</span></article>)}</section><div className="logistics-panels"><section className="logistics-panel" id="drivers"><p className="eyebrow">Field team</p><h2>Driver availability</h2><div className="driver-row"><i>SA</i><span><strong>Success A.</strong><small>On route · Yenagoa</small></span><b>2 stops left</b></div><div className="driver-row"><i>TB</i><span><strong>Timi B.</strong><small>On route · Ogbia</small></span><b>4 stops left</b></div><div className="driver-row"><i className="available">EN</i><span><strong>Emeka N.</strong><small>Available · Dispatch hub</small></span><b>Assign</b></div></section><section className="logistics-panel" id="zones"><p className="eyebrow">Coverage map</p><h2>Delivery zones</h2><div className="zone-map"><span>Yenagoa <b>8</b></span><span>Ogbia <b>3</b></span><span>Nembe <b>1</b></span></div><small className="zone-note">Active routes across 6 LGAs</small></section></div></section>{bookingOpen && <div className="overlay" onClick={() => setBookingOpen(false)}><section className="booking-modal" onClick={(event) => event.stopPropagation()}><button className="close" onClick={() => setBookingOpen(false)} aria-label="Close">×</button>{booked ? <div className="modal-success"><span>✓</span><p className="eyebrow">Delivery booked</p><h2>A driver is on it.</h2><p>Your dispatch request has been added to the pickup queue.</p><button className="primary-button" onClick={() => setBookingOpen(false)}>Done <span>→</span></button></div> : <><p className="eyebrow">New dispatch request</p><h2>Book a delivery.</h2><form className="dashboard-form" onSubmit={(event) => { event.preventDefault(); setBooked(true) }}><label>Pickup location<input required placeholder="Farm, hub, or seller address" /></label><label>Delivery destination<input required placeholder="Customer address or area" /></label><div className="form-grid"><label>Package type<select defaultValue="produce"><option value="produce">Fresh produce</option><option value="cooler">Cooler / seafood</option><option value="bulk">Bulk goods</option></select></label><label>Preferred time<select defaultValue="today"><option value="today">Today · 2 - 5 PM</option><option value="tomorrow">Tomorrow · 9 AM - 12 PM</option></select></label></div><button className="primary-button full" type="submit">Add to dispatch queue <span>↗</span></button></form></>}</section></div>}</main>
}
...
function App() {
  ...
  const removeFromCart = (indexToRemove) => setCart((current) => current.filter((_, index) => index !== indexToRemove))
  const toggleFavorite = (productName) => setFavorites((current) => current.includes(productName) ? current.filter((name) => name !== productName) : [...current, productName])
  ...
  useEffect(() => { const interval = setInterval(() => setSpotlightIndex((current) => (current + 1) % homeSpotlights.length), 4500); return () => clearInterval(interval) }, [])
  ...
  if (showLogistics) return <LogisticsDashboard onBack={() => setShowLogistics(false)} />
  if (currentPage === 'categories') return <CategoriesPage categories={categories} onBack={() => setCurrentPage('home')} onSelectCategory={() => setCurrentPage('deals')} />
  if (currentPage === 'deals') return <ProductCollectionPage title="Deals worth bringing home." eyebrow="Fresh offers, local prices" description="Save on the freshest picks from Bayelsa farmers, fishermen, and food producers." products={products.filter((product) => product.old || /save|deal|best seller/i.test(product.badge))} searchQuery={searchQuery} cart={cart} total={total} favorites={favorites} onBack={() => setCurrentPage('home')} onAddToCart={addToCart} onRemoveFromCart={removeFromCart} onToggleFavorite={toggleFavorite} onClearCart={() => setCart([])} />
  if (currentPage === 'new-arrivals') return <ProductCollectionPage title="New on the market." eyebrow="Just landed" description="Meet the latest products added by verified farmers and producers in our community." products={products} cart={cart} total={total} favorites={favorites} onBack={() => setCurrentPage('home')} onAddToCart={addToCart} onRemoveFromCart={removeFromCart} onToggleFavorite={toggleFavorite} onClearCart={() => setCart([])} />
  ...
  return (
    <main>
      <div className="topline"><span>📍 Yenagoa, Bayelsa</span><span>Free delivery on orders over ₦20,000 · Mon - Sun: 8:00 AM - 10:00 PM</span><span>☎ (123) 456-7890</span></div>
      <nav className="nav"><a className="brand" href="#top"><span className="brand-mark">✳</span><span>AgriMarket <i>Bayelsa</i></span></a><div className="market-search"><span>⌕</span><input aria-label="Search products" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Search for products, categories..." /><select aria-label="Search category"><option>All categories</option><option>Fresh fish</option><option>Vegetables</option><option>Fruits</option></select><button aria-label="Submit search" onClick={() => setCurrentPage('deals')}>⌕</button></div><div className="nav-actions"><button className="logistics-link" onClick={() => setShowLogistics(true)}>Logistics</button><button className="dashboard-link" onClick={openDashboard}>My dashboard</button><button className="icon-button" onClick={() => setShowCart(true)} aria-label="Open cart">♧<sup>{cart.length}</sup></button><button className="join-button" onClick={beginOnboarding}>Join AgriMarket <span>↗</span></button></div></nav>
      <div className="category-nav"><button onClick={() => setCurrentPage('categories')}>☰ <span>Shop by categories</span></button><a href="#top">Home</a><button className="category-nav-link" onClick={() => setCurrentPage('categories')}>Categories⌄</button><button className="category-nav-link" onClick={() => setCurrentPage('deals')}>Deals</button><button className="category-nav-link" onClick={() => setCurrentPage('new-arrivals')}>New arrivals</button><a href="#farmers">Meet our farmers</a><a href="#how">How it works</a><a href="#farmers">Sell with us</a><strong>✦ Flash deals</strong></div>
      <section className="hero" id="top"><div className="hero-copy"><p className="eyebrow"><span className="dot"></span> Freshness you can trust</p><h1>Fresh groceries,<br /><em>better living.</em></h1><p className="hero-text">Get the freshest produce, fish, and daily essentials delivered to your doorstep from Bayelsa’s trusted farmers.</p><div className="hero-actions"><button className="primary-button" onClick={() => setCurrentPage('new-arrivals')}>Shop now <span>↗</span></button><button className="outline-button" onClick={() => setCurrentPage('deals')}>Explore deals</button></div><div className="hero-meta"><span>✧ Farm fresh</span><span>⌁ Fast delivery</span><span>◉ Secure payment</span><span>↺ Easy returns</span></div></div><div className="hero-art"><div className="hero-photo"></div><div className="hero-offer">UP TO<strong>30%</strong>OFF</div><div className="hero-caption"><span>Harvested fresh.<br /><b>Delivered with care.</b></span></div></div></section>
      ...
      <section className="market-pulse" aria-label="Market highlights"><div className="pulse-intro"><span className="pulse-live"><i></i> Live market</span><strong>What’s fresh today</strong><small>Updated from local sellers</small></div><div className="pulse-feature" key={spotlightIndex}><span className="pulse-icon">{homeSpotlights[spotlightIndex].icon}</span><div><p>{homeSpotlights[spotlightIndex].label}</p><strong>{homeSpotlights[spotlightIndex].title}</strong><small>{homeSpotlights[spotlightIndex].detail}</small></div><button onClick={() => setCurrentPage(homeSpotlights[spotlightIndex].action)}>Explore <span>↗</span></button></div><div className="pulse-dots">{homeSpotlights.map((spotlight, index) => <button className={index === spotlightIndex ? 'active' : ''} onClick={() => setSpotlightIndex(index)} aria-label={`Show ${spotlight.title}`} key={spotlight.title}></button>)}</div></section>
      ...
      {showCart && <div className="overlay" onClick={() => setShowCart(false)}><aside className="drawer" onClick={(event) => event.stopPropagation()}><button className="close" onClick={() => setShowCart(false)}>×</button><p className="eyebrow">Your basket</p><h2>Fresh picks.</h2>{cart.length === 0 ? <div className="empty"><span>🧺</span><p>Your basket is waiting for something good.</p><a href="#shop" onClick={() => setShowCart(false)}>Browse the market →</a></div> : <>{cart.map((item, index) => <div className="cart-item" key={`${item.name}-${index}`}><img src={item.image} alt="" /><div><strong>{item.name}</strong><small>{item.price}</small></div><button className="cart-remove" onClick={() => removeFromCart(index)} aria-label={`Remove ${item.name} from cart`}>×</button></div>)}<div className="cart-total"><span>Total</span><strong>₦{total.toLocaleString()}</strong></div><button className="primary-button full" onClick={() => { setShowCart(false); setShowCheckout(true) }}>Continue to checkout →</button></>}</aside></div>}
      {showCheckout && <CheckoutPanel cart={cart} total={total} onClose={() => setShowCheckout(false)} onComplete={() => setCart([])} />}
